...
StaFile = 'sta_obs.txt';   % file containing the station information
...
datadir = '/work/li_chao/work/Axial_Seamount/'; % folder containing all sac data for all stations (data must be named with Julian day ...)
PeriodBand = [0.1 5]; % multiple connecting period bands for temoporal normalization of ambient noise data
...
yearrange = [2015 2015]; % year of the data
%dayrange = [114 114]; % range of days in a year for CF calculation
dailyrange = [114 129] ; % range of daily data 
dayeruption = 114 ;
MaxLagTime = 20;  % maximum time length (in seconds) for each side of CFs
...
outCFdir = './AXCC1_AXEC2/'; % output folder for CFs

IndexParfor=0; % =1, use parallel computing; otherwise don't use parallel computing.
n_parpool=2; % for parfor, use n_parpool cores in one CPU.(only be used in case of IndexParfor=1)
%__________________________________________________________________________
...
for day = dailyrange(1):dailyrange(2)
    daily = day
    dayrange = [day,day]
    ...
    %%
    if IndexParfor==1 % use parallel computing
        
        %matlabpool local n_parpool; % version <= matlab2012a
        parpool('local',n_parpool); % version > matlab2012a
        
        parfor k=1:NumStaPair
            daily_noisecorr_sac_parfor(k,i_j_sta(k,:),parameter_file_tmp);
        end
