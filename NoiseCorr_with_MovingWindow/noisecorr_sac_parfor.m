    ...
    for day = dayrange(1):dayrange(2) % loop for each julian day in a year
        ...
        if  sta1_dataok && sta2_dataok % sac data file for station 1 & 2 exists
            ...
            if (1/sta1.DELTA) > fsNew
                ...
                display([DecimateR_org,DecimateR])
            ...
            display([length(sta1.DATA1),length(sta2.DATA1)])
            % tic
...
if ncf > 0
    ...
    for nd = 1:ncf
        ...
        if max(dailyCFmax) > 0
            stackCF = stackCF + CFdata(nd).NCF./dailyCFmax; % stack normalized daily CF for each period band
            nnday = nnday + 1;
        end
    end
    disp(nnday)
