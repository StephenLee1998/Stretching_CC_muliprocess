...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function noisecorr_sac_parfor(k,i_j,parameter_file_tmp)
...
for year= yearrange(1):yearrange(2);
    ...
    for day = dayrange(1):dayrange(2) % loop for each julian day in a year
        ...
        for hour = 1:12
            ...
            % -------------------------------------------------------------------------------------------------------
            seisfile1 = [datadir allsta(i).name '/' yearstr '/' allsta(i).name '_' cmp1 '_' daystr '_' hourstr '.sac'];
            seisfile2 = [datadir allsta(j).name '/' yearstr '/' allsta(j).name '_' cmp2 '_' daystr '_' hourstr '.sac'];
            % -------------------------------------------------------------------------------------------------------
            %display([seisfile1]);
            %display([seisfile2]);
            ...
            if  sta1_dataok && sta2_dataok % sac data file for station 1 & 2 exists
                ...
                display(['Now processing: ' yearstr '  '  daystr '  ' hourstr '    ...... ']);
