...
StaFile = './sta/sta_AXCC1_AXCC1H.txt';   % file containing the station information
...
indexCorrMethod = 1; % = 1: one-bit cross correlation; = 2: temporal normalization cross-correlation
yearrange = [2017 2020]; % year of the data
dailyrange = [1 367]; % range of days in a year for CF calculation
dayeruption = 0 ;
MaxLagTime = 100;  % maximum time length (in seconds) for each side of CFs
...
outCFdir = '../NCFs/Test/AXCC1_AXCC1_15d/'; % output folder for CFs

IndexParfor=1; % =1, use parallel computing; otherwise don't use parallel computing.
n_parpool=6; % for parfor, use n_parpool cores in one CPU.(only be used in case of IndexParfor=1)
%__________________________________________________________________________
...
for iyear = yearrange(1):yearrange(2);
    ...
    parameter_file_tmp=['parameter_file_tmp_daily.mat'];
